...
from mrcnn import contextual_attention
...
# Code adopted from:
# https://github.com/fchollet/deep-learning-models/blob/master/resniwqet50.py
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    x = contextual_attention.Contextual_Attention(1, 64, 64, 1024, intermediate_dim=256, base_name='attn1_')(x)
    x = identity_block(x, 3, [256, 256, 1024], stage=4, block=chr(98 + (block_count-2)), train_bn=train_bn) 
    x = contextual_attention.Contextual_Attention(1, 64, 64, 1024, intermediate_dim=256, base_name='attn2_')(x)
    C4 = x = identity_block(x, 3, [256, 256, 1024], stage=4, block=chr(98 + (block_count-1)), train_bn=train_bn)
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, gt_orientations, config):
    ...
    gt_orientations: [1, MAX_GT_INSTANCES] of boolean type.
    ...
    gt_orientations = tf.gather(gt_orientations, tf.compat.v1.where(non_zeros)[:, 0], axis=0, name="trim_gt_orientations")
    ...
    orientations = tf.pad(tensor=roi_gt_orientations, paddings=[(0, N + P), (0, 0)])
    return rois, roi_gt_class_ids, deltas, masks, orientations
...
class DetectionTargetLayer(KL.Layer):
    ...
    def compute_output_shape(self, input_shape):
        return [
            ...
            (None, self.config.TRAIN_ROIS_PER_IMAGE, 1)  # gt_orientations
        ]
...
def refine_detections_graph(rois, probs, deltas, window, orientations, config):
    ...
    # Arrange output as [N, (y1, x1, y2, x2, class_id, score, x, y)]
    # Coordinates are normalized.
...
class DetectionLayer(KL.Layer):
    ...
    def call(self, inputs):
        ...
        detections_batch = utils.batch_slice(
            ...
            lambda x, y, w, z, o: refine_detections_graph(x, y, w, z, o, self.config),
            self.config.IMAGES_PER_GPU)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # orientation
    # xo = KL.TimeDistributed(KL.Conv2D(fc_layers_size, (pool_size, pool_size), padding="valid"),
    #                        name="mrcnn_orientation_conv1")(x)
    # xo = KL.TimeDistributed(BatchNorm(), name='mrcnn_orientation_bn1')(xo, training=train_bn)
    # xo = KL.Activation('relu')(xo)
    # xo = KL.TimeDistributed(KL.Conv2D(fc_layers_size, (1, 1)),
    #                        name="mrcnn_orientation_conv2")(xo)
    # xo = KL.TimeDistributed(BatchNorm(), name='mrcnn_orientation_bn2')(xo, training=train_bn)
    # xo = KL.Activation('relu')(xo)
    # sharedo = KL.Lambda(lambda x: K.squeeze(K.squeeze(x, 3), 2),
    #                    name="pool_squeeze_orientation")(xo)
    ...
    x = KL.TimeDistributed(KL.Dense(num_classes * 1, activation='linear'),
                           name='mrcnn_orientation_fc')(shared)
    s = K.int_shape(x)
    if s[1] == None:
        mrcnn_orientation = KL.Reshape((-1, num_classes, 1), name="mrcnn_orientation")(x)
    else:
        mrcnn_orientation = KL.Reshape((s[1], num_classes, 1), name="mrcnn_orientation")(x)

    return mrcnn_class_logits, mrcnn_probs, mrcnn_bbox, mrcnn_orientation
...
def fpn_orientation_graph(rois, feature_maps, image_meta,
                         pool_size, num_classes, train_bn=True,
                         fc_layers_size=1024):
    """
    Builds the computation graph of the feature pyramid network classifier
    ...
    Returns:
        mrcnn_orientation s[1] 2
    """
    # ROI Pooling
    # Shape: [batch, num_boxes, pool_height, pool_width, channels]
    x = PyramidROIAlign([pool_size, pool_size],
                        name="roi_align_classifier_orientation")([rois, image_meta] + feature_maps)
    # Two 1024 FC layers (implemented with Conv2D for consistency)
    x = KL.TimeDistributed(KL.Conv2D(fc_layers_size, (pool_size, pool_size), padding="valid"),
                           name="mrcnn_class_conv1_orientation")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_class_bn1_orientation')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    x = KL.TimeDistributed(KL.Conv2D(fc_layers_size, (1, 1)),
                           name="mrcnn_class_conv2_orientation")(x)
    x = KL.TimeDistributed(BatchNorm(), name='mrcnn_class_bn2_orientation')(x, training=train_bn)
    x = KL.Activation('relu')(x)
...
def mrcnn_orientation_loss_graph(target_orientation, target_class_ids, pred_orientation):
    """Loss for the orientation head of Mask RCNN.
    """
    # Loss
    # print("pred_orientation", pred_orientation.shape)
    target_class_ids = K.reshape(target_class_ids, (-1,))
    target_orientation = K.reshape(target_orientation, (-1, 1))
    pred_orientation = K.reshape(pred_orientation, (-1, K.int_shape(pred_orientation)[2], 1))
    # Only positive ROIs contribute to the loss. And only
    # the right class_id of each ROI. Get their indices.
    positive_roi_ix = tf.compat.v1.where(target_class_ids > 0)[:, 0]
    positive_roi_class_ids = tf.cast(
    ...
    # Gather the deltas (predicted and true) that contribute to loss
    target_orientation = tf.gather(target_orientation, positive_roi_ix)
    pred_orientation = tf.gather_nd(pred_orientation, indices)
    # print(target_orientation.shape)
    # print(pred_orientation.shape)
    # smooth L1 Loss
    # less_than_one = K.cast(K.less(diff, 1.0), "float32")
    # loss = (less_than_one * 0.5 * diff**2) + (1 - less_than_one) * (diff - 0.5)
    # loss = K.switch(tf.size(loss) > 0, K.mean(loss), tf.constant(0.0))
    # angle loss
    loss = tf.abs(tf.atan2(tf.sin(target_orientation - pred_orientation), tf.cos(target_orientation - pred_orientation)))
    loss = K.switch(tf.size(input=loss) > 0, K.mean(loss), tf.constant(0.0))
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    orientations = dataset.load_orientations(image_id)
    ...
    orientations = orientations[_idx, :]
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, gt_orientations, config):
    ...
    gt_masks: [height, width, instance count] Ground truth masks. Can be full
              size or mini-masks.
    gt_orientation: [instance count, 1]
    ...
    orientations: [TRAIN_ROIS_PER_IMAGE, (dy, dx)]
    """
    ...
    gt_orientations = gt_orientations[instance_ids, :]
    ...
    orientations = np.zeros((config.TRAIN_ROIS_PER_IMAGE, 1), dtype=np.float32)
    ...
    for i in pos_ids:
        ...
        orientations[i, :] = gt_orientations[gt_id, :]

    return rois, roi_gt_class_ids, bboxes, masks, orientations
        ...
        try:
            ...
            if random_rois:
                ...
                if detection_targets:
                    rois, mrcnn_class_ids, mrcnn_bbox, mrcnn_mask, mrcnn_orientation =\
                        build_detection_targets(
                            rpn_rois, gt_class_ids, gt_boxes, gt_masks, gt_orientations, config)
            ...
            if b == 0:
                ...
                batch_gt_orientations = np.zeros(
                    (batch_size, config.MAX_GT_INSTANCES, 1), dtype=gt_orientations.dtype)
                ...
                if random_rois:
                    ...
                    if detection_targets:
                        ...
                        batch_mrcnn_orientations = np.zeros((batch_size,) + mrcnn_orientation.shape, dtype=mrcnn_orientation.shape)
            ...
            if random_rois:
                ...
                if detection_targets:
                    ...
                    batch_mrcnn_orientations[b] = mrcnn_orientation
            b += 1

            # Batch full?
            if b >= batch_size:
                inputs = [batch_images, batch_image_meta, batch_rpn_match, batch_rpn_bbox,
                          batch_gt_class_ids, batch_gt_boxes, batch_gt_masks, batch_gt_orientations]
                outputs = []
                ...
                if random_rois:
                    ...
                    if detection_targets:
                        ...
                        outputs.extend(
                            [batch_mrcnn_class_ids, batch_mrcnn_bbox, batch_mrcnn_mask, batch_mrcnn_orientations])
                # print(batch_mrcnn_orientations)
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir):
        ...
        model_dir: Directory to save training logs and trained model
        """
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            from tensorflow.keras.layers import Lambda as TFLAMBDA
            print('HERE')
            gt_boxes = TFLAMBDA(lambda x: norm_boxes_graph(
            ...
            if config.USE_MINI_MASK:
                ...
                input_gt_masks = KL.Input(
                    ...
                    name="input_gt_masks", dtype=bool)
            # 4. GT orientation
            input_gt_orientations = KL.Input(shape=[None, 1], name="input_gt_orientation", dtype=tf.float32)
        ...
        if mode == "training":
            ...
            #anchors = TFLAMBDA(lambda x: tf.Variable(anchors), name="anchors")(input_image)
        else:
        ...
        if mode == "training":
            ...
            rois, target_class_ids, target_bbox, target_mask, target_orientation =\
                DetectionTargetLayer(config, name="proposal_targets")([
                    target_rois, input_gt_class_ids, gt_boxes, input_gt_masks, input_gt_orientations])
            ...
            # mrcnn_orientation = \
            #     fpn_orientation_graph(rois, mrcnn_feature_maps, input_image_meta,
            #                          config.POOL_SIZE, config.NUM_CLASSES,
            #                          train_bn=config.TRAIN_BN,
            #                          fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)
            ...
            orientation_loss = KL.Lambda(lambda x: mrcnn_orientation_loss_graph(*x), name="mrcnn_orientation_loss")(
                [target_orientation, target_class_ids, mrcnn_orientation])

            # Model
            inputs = [input_image, input_image_meta,
                      input_rpn_match, input_rpn_bbox, input_gt_class_ids, input_gt_boxes, input_gt_masks, input_gt_orientations]
            if not config.USE_RPN_ROIS:
                inputs.append(input_rois)

            outputs = [rpn_class_logits, rpn_class, rpn_bbox,
                       mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask, mrcnn_orientation,
            ...
            # mrcnn_orientation = \
            #     fpn_orientation_graph(rpn_rois, mrcnn_feature_maps, input_image_meta,
            #                          config.POOL_SIZE, config.NUM_CLASSES,
            #                          train_bn=config.TRAIN_BN,
            #                          fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)
            # Detections
            ...
            detections = DetectionLayer(config, name="mrcnn_detection")(
                [rpn_rois, mrcnn_class, mrcnn_bbox, mrcnn_orientation, input_image_meta])
            ...
            model = KM.Model([input_image, input_image_meta, input_anchors],
                             [detections, mrcnn_class, mrcnn_bbox, mrcnn_orientation,
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        # of the inner model because they have the model.
    ...
    def get_imagenet_weights(self):
        """Downloads ImageNet trained model from Keras.
    ...
    def compile(self, learning_rate, momentum):
        ...
        optimizer = SGD(
            ...
            clipnorm=self.config.GRADIENT_CLIP_NORM, decay=0.0001)
        ...
        loss_names = [
            ...
            "mrcnn_mask_loss", "mrcnn_orientation_loss"]
        ...
        # Skip gamma and beta model of batch normalization layers.
        reg_losses = [
        ...
        for name in loss_names:
            ...
            # self.keras_model.add_metric(lambda: layer, name=name)
            ...
            #self.keras_model.add_metric(lambda: loss, name=name)
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        layer_regex = {
            ...
            # "orientation"
            "orientation": r"(mrcnn_orientation\_.*)",
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        # network model are still random
        exclude_ix = np.where(
        ...
        if exclude_ix.shape[0] > 0:
            ...
            orientations = np.delete(orientations, exclude_ix, axis=0)
            N = class_ids.shape[0]
    ...
    def get_trainable_layers(self):
        """Returns a list of layers that have model."""
        layers = []
        # Loop through all layers
        for l in self.keras_model.layers:
            # If layer is a wrapper, find inner trainable layer
            l = self.find_trainable_layer(l)
            # Include layer if it has model
            if l.get_weights():
                layers.append(l)
        return layers
...
def unmold_image(normalized_images, config):
    ...
    return (normalized_images[:, :, :3] + config.MEAN_PIXEL).astype(np.uint8)
